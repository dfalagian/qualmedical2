-- Corregir el trigger para que funcione con el tipo ENUM document_type
...
BEGIN
  ...
  IF (TG_OP = 'UPDATE' AND OLD.status IS DISTINCT FROM NEW.status) OR TG_OP = 'INSERT' THEN
    ...
    -- CAST document_type a text para la comparación
    SELECT COUNT(DISTINCT document_type::text)
    INTO approved_count
    FROM public.documents
    WHERE supplier_id = NEW.supplier_id
      AND document_type::text = ANY(required_doc_types)
      AND status = 'aprobado';
