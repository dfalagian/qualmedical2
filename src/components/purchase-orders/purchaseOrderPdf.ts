...
function createDoc(orderData: PurchaseOrderPdfData) {
  const doc = new jsPDF();
  const pageWidth = doc.internal.pageSize.getWidth();

  // Header (QualMedical brand color)
  ...
  doc.setFontSize(10);
  doc.setFont("helvetica", "bold");
  doc.text(`No. de Orden: ${orderData.orderNumber}`, pageWidth - 14, 20, {
    align: "right",
  });
  doc.setFont("helvetica", "normal");
  doc.text(`Fecha: ${orderData.createdAt.toLocaleDateString("es-MX")}`, pageWidth - 14, 28, {
    align: "right",
  });
  ...
  const tableData = orderData.items.map((item, index) => [
    ...
    item.hasIva
      ? `$${item.ivaAmount.toLocaleString("es-MX", { minimumFractionDigits: 2 })}`
      : "$0.00",
  ...
  doc.setFont("helvetica", "normal");
  doc.text(
    `$${orderData.subtotal.toLocaleString("es-MX", { minimumFractionDigits: 2 })}`,
    pageWidth - 14,
    finalY,
    { align: "right" }
  );
  ...
  doc.setFont("helvetica", "normal");
  doc.text(
    `$${orderData.totalIva.toLocaleString("es-MX", { minimumFractionDigits: 2 })}`,
    pageWidth - 14,
    finalY + 6,
    { align: "right" }
  );
  ...
  doc.text(
    `$${orderData.total.toLocaleString("es-MX", { minimumFractionDigits: 2 })}`,
    pageWidth - 14,
    finalY + 14,
    { align: "right" }
  );
...
export function createPurchaseOrderPdfBlob(orderData: PurchaseOrderPdfData): Blob {
  const doc = createDoc(orderData);
  return doc.output("blob") as Blob;
}

export function downloadPurchaseOrderPdf(orderData: PurchaseOrderPdfData) {
  const doc = createDoc(orderData);
