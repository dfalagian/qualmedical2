...
export const PurchaseOrderPDFViewer = ({
  ...
  const handlePrint = () => {
    // Regenerar el PDF con autoPrint y abrirlo
    const doc = new jsPDF();
    const pageWidth = doc.internal.pageSize.getWidth();

    // Copiar la lógica de generación de PDF
    doc.setFillColor(0, 128, 105);
    doc.rect(0, 0, pageWidth, 40, "F");
    doc.setTextColor(255, 255, 255);
    doc.setFontSize(11);
    ...
    // Agregar autoPrint para que imprima automáticamente al abrir
    doc.autoPrint();
    
    // Abrir en nueva pestaña (no popup) para evitar bloqueo
